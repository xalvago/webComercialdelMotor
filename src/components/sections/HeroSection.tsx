...
function SummerBannerContent({ sb, large = false }: { sb: SummerBanner; large?: boolean }) {
  const titleSize = large ? "text-base" : "text-sm";
  const monthsSize = large ? "text-sm" : "text-xs";
  const iconSize = large ? 32 : 24;
  const zoneSize = large ? "text-xl" : "text-lg";
  const hoursSize = large ? "text-lg" : "text-base";
  return (
    <>
      ...
      <span
        className={`block text-[#EF0029]/70 ${monthsSize} uppercase tracking-[0.15em] mb-3`}
        style={{ fontFamily: "'JetBrains Mono', monospace" }}
      >
        {sb.months}
      </span>
